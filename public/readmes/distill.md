...
每个回合完成（`agent/turn-stopping` 触发）时，插件会收集自上次蒸馏检查点以来新增的人类 `user/message` 事件；数量达到 `minUserMessages` 后，派发一个后台反省子代理（Hermes Agent 后台反省的形态：受限工具集的全新子代理，在回合之后运行，从不与用户任务争抢）。子代理的提示词携带 Hermes 策展课程、消息窗口帧和可更新技能列表；其工具集白名单只保留 `skill` 查看器，最终答案通过结构化输出契约捕获。派发前会发出非持久化的 Cordis `distill/review-request` 通知，携带精确的路由、提示词、工具白名单和 token 上限；该通知不会写入会话日志。
...
主对话不注册任何工具或提示，插件从不改变其表面。唯一的模型可见效果是间接的：后台运行一个带 `skill` 工具的反省子代理（它看到与主 agent 相同的目录，可在提议前查看任意技能内容），写入或更新的技能会在后续轮次出现在 `dsh-tool-skill` 目录中。反省派发通过仅运行期的 `distill/review-request` 通知暴露给观察者，不写入会话日志，也不进入对话循环；需要跨进程审计的部署应由监听方自行持久化该通知。
...
- **检查点仅限当前进程** — 检查点保存在内存中；恢复或 fork 的会话从 `Session.firstLiveSeq` 开始，只处理本进程新增的消息。上一个进程中尚未达到阈值的消息不会结转。
- **既有受损日志需单独修复** — 本版本不再写入 `session/distill-review-request`，但已经含有该未知事件的日志仍需为事件信封补上 `ignorable: true` 或使用宿主提供的修复工具后才能重新加载。
