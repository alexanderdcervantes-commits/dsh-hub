...
<p align="center">
  <img src="https://raw.githubusercontent.com/poiuyjie/dsh-vision-opencode/bf8813980f3b4a51c6510ac29b97540bf619f861/assets/demo.png" alt="dsh-vision-opencode 演示" width="860" />
</p>
...
- 输入框右侧「识图模型」下拉选择器（自动列出所有供应商中声明了图片输入的模型），样式与交互复刻 DSH 官方模型选择器（含分组标题、尾部对勾、方向键导航、失败重试与 Toast），触发器带「Vision」前缀
- 设置页「Vision」独立分区（设置 → Vision）：以官方模型页同款卡片管理插件自管的 Vision 模型（增删改查 + 设为当前），与系统模型列表解耦
- 运行时 skill `vision-image-analysis`：按需指导主模型调用 `vision_read_image` 做 OCR / 图表 / 场景理解；未启用 DSH skill 服务时自动退化为工具与系统提示词
- 用户在聊天里发图片时**自动转换**：识图进度临时显示在选择器触发器的「Vision」槽位（运行中为官方同款微光动效），结束后只用 DSH 原生 `notice` 留下一条完成或失败记录
...
vision-opencode:
  ...
  visionModels: []          # 插件自管 Vision 模型列表（设置 → Vision 图形化管理，含 provider/model/name/description）
...
> 图形化配置：打开「设置 → Vision」（与“模型”并列的独立标签页），以官方同款卡片增删改查 Vision 模型并设为当前；也可直接编辑 `settings.yaml` 后重启。
